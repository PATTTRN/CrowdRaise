...
export default function EditCollection({ params }: { params: Promise<{ id: string }> }) {
    ...
    const fetchCollection = async () => {
      try {
        ...
        setFormData({
          ...
          goal: col.goal?.toString() || '',
          eventDate: col.eventDate ? new Date(col.eventDate).toISOString().split('T')[0] : '',
          receiverName: col.receiverName || '',
          suggestedAmounts: col.suggestedAmounts?.join(', ') || '',
          description: col.description,
          fullStory: col.fullStory,
          fundUsage: col.fundUsage?.[0]?.description || '',
        });
        setExistingImages(col.images || []);
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (const file of imageFiles) {
        ...
        const uploadRes = await upload({ ...authParams, file, fileName: file.name, folder: "/crowdraise/collections" });
        uploadedImages.push({ url: uploadRes.url, publicId: uploadRes.fileId });
      }

      const finalImages = [...existingImages.map(img => ({ url: img.url, publicId: img.publicId })), ...uploadedImages];
  ...
  const inputCls = `w-full px-4 py-3 rounded-xl border border-white/10 bg-white/5 text-white focus:outline-none focus:border-pink-500 transition-all`;
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 bg-white/5 p-8 rounded-3xl border border-white/10 backdrop-blur-xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <select name="category" value={formData.category} onChange={handleInputChange} className={inputCls} required>
                {config.categories.map(c => <option key={c} value={c} className="bg-neutral-900">{c}</option>)}
              </select>
            </div>
            <div>
          ...
          <div>
            ...
            <div className="grid grid-cols-3 sm:grid-cols-5 gap-4 mb-4">
              {existingImages.map((img) => (
                <div key={img._id} className="relative aspect-square rounded-lg overflow-hidden border border-white/10">
                  <img src={img.url} className="w-full h-full object-cover" />
                  <button type="button" onClick={() => removeExistingImage(img._id)} className="absolute top-1 right-1 bg-black/60 p-1 rounded-full text-white/80 hover:text-white"><i className="fas fa-times text-xs"></i></button>
                </div>
              ))}
              {imagePreviews.map((prev, i) => (
                <div key={i} className="relative aspect-square rounded-lg overflow-hidden border border-pink-500/50">
                  ...
                  <button type="button" onClick={() => removeNewImage(i)} className="absolute top-1 right-1 bg-black/60 p-1 rounded-full text-white/80 hover:text-white"><i className="fas fa-times text-xs"></i></button>
                </div>
              ))}
          ...
          <div className="flex gap-4 pt-4">
            <button type="button" onClick={() => router.back()} className="flex-1 py-4 rounded-xl border border-white/10 text-white font-bold hover:bg-white/5 transition-all">Cancel</button>
            <button type="submit" disabled={isSubmitting} className="flex-[2] py-4 rounded-xl bg-gradient-to-r from-pink-500 to-rose-500 text-white font-bold shadow-lg shadow-pink-500/20 hover:scale-[1.02] transition-all disabled:opacity-50">
